...
Date: 2026-09-14. Inputs: project brief, `IMPLEMENTATION_RULES.md`, `REPOSITORY_STRUCTURE.md`, `V1_SCHEMA.sql`, `domain.ts`, `events.ts`, `company-deep-research.yaml` (now under `docs/spec/`). Per brief §32 this report preceded any code. Status: schema fixes I.1–I.16 are implemented in `packages/db/migrations/20260914000000_schema_v1_1.sql` and verified by `packages/db/tests/schema_v1_1.test.sql`. Contract fixes I.17–I.25 are implemented in `packages/domain`, `packages/events`, `packages/schemas` and `packages/research`, verified by `pnpm test`. Layout decisions I.26–I.28 are recorded in `01-repository-layout.md`; the import-boundary lint of I.28 is deferred until `apps/web` exists. The three rule change proposals below remain proposals awaiting a decision, except that the rule 7 extension is already enforced in `packages/schemas`. Phase J.2 (identity) is implemented: `packages/identity`, `packages/db` repositories, and seed 001 with six rare-earth issuers. Phase J.3 (ingestion) is implemented: `packages/ingest` holds the EDGAR connector and the `us-gaap` concept map, `packages/db/src/ingest-repository.ts` writes immutable content-hashed document versions and promotes XBRL values with no model in the path, and migration `20260915000000_source_registry_unique.sql` adds the source-name uniqueness the upsert needs. The live SEC fetch is unverified: SEC returns 403 unless `SEC_USER_AGENT` carries a real contact address, which is the operator's to supply. Phase J.4 (analytics) is implemented: `services/analytics` holds pure ratio, DCF, reverse-DCF and multiples functions behind a registry and a FastAPI `POST /calc/{method}`, and `packages/db/src/calc-repository.ts` stores each invocation as a `valuation.calculation_runs` row and each result as a promoted `CALCULATED` fact linked through `evidence.fact_derivations` to the revisions it was computed from. Two decisions are worth recording. `factEpistemicStatus` no longer requires a source tier: a calculated value has no source of its own, its inputs carry the tiers, and demanding one invited a fabricated number; a value read off a source with no tier now throws instead. And a calculation is stored as an event, not a registry entry, so re-running writes a new run row while writing no new fact revision. Phase J.5 (LLM gateway) is implemented: `packages/ai` holds one provider adapter (OpenRouter, at the request of the operator), a deliberately narrow Zod to JSON Schema converter and tier routing, and `packages/db/src/model-repository.ts` logs every call to `research.model_runs` and caches responses in `research.model_cache` (migration `20260916000000_model_cache.sql`). The cache is a separate table from the log on purpose: eviction must not destroy the audit trail. The live provider call is unverified because no `OPENROUTER_API_KEY` is available here, but the phase gate does not need one: a replay parses cached bytes through the same schema the live path uses, and the integration test proves it with the transport wired to throw. Cost is recorded, not estimated: OpenRouter reports what each call was charged, so there is no price table to keep current and a call the provider does not cost records null. Phase J.6 (module runtime) is implemented: `packages/research/src/runtime.ts` holds the context, claim and evidence contracts plus the pure output validation, `packages/research/src/implementations.ts` holds the modules, and `packages/db/src/research-repository.ts` freezes the snapshot, registers module definitions and prompt versions, runs the DAG level by level and writes module runs, claims and claim evidence append-only. Two decisions are worth recording. The orchestrator is a plain async function with a `step` seam rather than an Inngest function: Inngest is not in the repository and adding a server to run one module DAG would have been infrastructure ahead of need, so `Step` is injected and the default implementation calls through -- wrapping it in `step.run` later changes one line and no module. And the recipe carries `business_model` alongside the three modules the phase names, because the module registry makes it a dependency of `commodity_exposure` and the registry is the single source of DAG truth (I.18); satisfying the dependency was preferable to editing it away. The citation check is the gate: a quote must appear in the chunk it cites, exactly or modulo whitespace, and every cited id must be inside the frozen snapshot, so a module cannot cite evidence the run never read. Phase J.7 (verification) is implemented: `packages/research/src/verification.ts` holds the deterministic rules -- quote containment, numbers against cited evidence, source tier, and contradiction on a shared `claim_key` -- and `packages/db/src/verification-repository.ts` runs them over the stored rows of a research run, writes `research.verification_runs` and a `research.verification_checks` row per rule per claim, and demotes failures to `CONTRADICTED`. Three decisions are worth recording. Verification reads stored rows rather than module output, duplicating a check the runtime already performs, because a check that runs only on the way in cannot catch a row that arrived another way and cannot be re-run later against a claim whose source has since changed. Contradiction is scoped to claims within one run: the same `claim_key` restated by a later run is a restatement, not a disagreement, and comparing across runs belongs to monitoring (J.10). And promotion is absent, which is a deviation from report G worth naming: G gives the validator authority to assign `VERIFIED`, but invariant C.7 as implemented forbids any claim with `created_by = 'llm'` from holding that status, so a validator cannot promote one in place -- it would have to append a system-authored claim superseding the model's. Nothing reads `VERIFIED` yet, so that is deferred to the phase that does rather than guessed at now. The number rule is also weaker than it looks on its own: a fabricated quote can contain the fabricated number citing it, so containment is what anchors the pair. Phase J.8 (decision) is implemented: `packages/research/src/decision.ts` holds the two deterministic gates -- the assumption policy that approves or refuses a proposed valuation input, and the anchoring check that refuses a thesis node resting on nothing -- and `packages/db/src/decision-repository.ts` runs policy, scenario, valuation and synthesis in that order over the stored output of a completed run, ending in a `research.thesis_versions` row with nodes and edges. Four decisions are worth recording. I.22 is honoured by splitting the old `valuation` module: `valuation_assumptions` is a real module in the recipe and proposes numbers as `proposed` assumption versions with a source claim, a range and a rationale, while approval, scenario membership, the calculation and the synthesis are steps outside the DAG, because each of them persists and a module may not (I.20). `final_synthesis` is likewise outside the recipe even though the registry declares it: it writes thesis rows rather than claims, and the three verification modules it depends on are implemented as the deterministic pass of J.7 over stored claims rather than as modules. The DCF is driven by four assumptions the model proposes -- discount rate, terminal growth, growth rate, projection years -- while the base cash flow, net debt and share count come from promoted fact revisions, so the model supplies judgement and the record supplies measurement; the projection is expanded from one approved rate deterministically rather than asked for as a list, which keeps every number in the calculation traceable to one approved row. And `VERIFIED` promotion, deferred at J.7, is still absent and is now positively justified rather than postponed: the decision layer needs to know whether a claim still stands, not whether it has been promoted, so the assumption policy refuses anything founded on a `CONTRADICTED` or `STALE` claim and names `VERIFIED` in the set it would accept. When promotion arrives it appends a system-authored claim rather than changing one in place, and nothing here has to change. Phase J.9 (web read models) is implemented: `packages/db/src/read-repository.ts` holds the queries behind the company page, the claim drill-down, the filing page and run status, and `apps/web` is a Next.js App Router app of server components that renders them, with Better Auth for email and password and migration `20260917000000_auth_minimal.sql` for its four tables. Six decisions are worth recording. Invariant C.9 is enforced by construction rather than by discipline: the read-model file never writes and computes no number that is not already stored, so a page can be stale but it cannot disagree with the record. "What changed" is derived on read from two immutable thesis versions rather than stored, because a difference described by the thing that produced it is not evidence of anything; nodes are matched across versions by `claim_key` or assumption code rather than by id, since a claim id is a per-run row and matching on it would report every restatement as new. The valuation range is `min` and `max` across the calculation runs of the same method rather than a stored band, so one scenario gives a point, a second widens it, and no code changes in between -- which is the difference between a read model and a report. There are two drill-down routes on purpose: a claim reaches its quote and the chunk of the stored filing that contains it, and a valuation input reaches the promoted fact revision's own source document, because a page that can only trace prose is not tracing the valuation. Better Auth guards one thing, the run trigger, because research state is global and reading it needs no account while starting a run spends money and writes rows; its tables are prefixed and deliberately not joined to `core.users` by a foreign key but matched on email, so an upgrade to the library cannot force a migration of the domain, and `research.runs.user_id` records who asked. And I.28 is settled, which is a deviation worth naming: the layout decision promised an eslint import boundary "when apps/web lands", and what landed instead is `tests/dependency-direction.test.ts`, a ring check over each workspace package's declared dependencies -- pnpm links strictly, so the declared graph is the import graph, and eslint would have added a toolchain to re-derive what the package manager already enforces. The run trigger itself runs research, verification and the decision in this process and returns as soon as the run row exists, which is honest under `next start` and is the first thing to move behind the `Step` seam if this is ever deployed somewhere that recycles. Phase J.10 (monitoring) is implemented: `packages/monitoring` holds the three drift rules as pure functions over stored rows, and `packages/db/src/monitoring-repository.ts` loads the rows they read, evaluates every enabled rule on the entity and writes `monitoring.alert_events`. Five decisions are worth recording. The drift report is derived on read from the latest thesis and the rows underneath it, never stored, for the same reason J.9 derives "what changed" on read: a difference described by the thing that produced it is not evidence of anything. Monitoring recomputes nothing -- it compares two stored calculation runs rather than running the engine again -- because a watch that recalculates its own evidence is a second opinion, and a second opinion that disagrees with the record is exactly what invariant C.9 exists to prevent. `new_primary_source` fires on document versions captured after the run behind the current thesis began reading, rather than on versions missing from that run's snapshot manifest, which is the more exact comparison and the wrong one: a document version with no chunks, an XBRL payload for instance, can never enter a snapshot and would therefore be new forever. `assumption_invalidated` follows the path the report names -- thesis node to claim to `claim_evidence` to the fact revision or document version cited -- and fires only for assumption nodes, while the drift report carries every affected node, because an amended filing under a driver is something to read and an approved number resting on a figure that has since moved is something to act on. And nothing is published: `intelligence.alert.triggered` has a contract in `packages/events` and no bus to carry it, so the `alert_events` row is the durable record and the event waits for the phase that needs one. Idempotence is structural rather than careful: every alert carries a dedupe key that is a function of its cause and the schema holds that column unique, so monitoring can run on any schedule at all. Phase J.11 (breadth) is implemented: the eight remaining model-facing modules are in `packages/research/src/implementations.ts` behind a second recipe, `DEEP_RECIPE`, which is the spec's `company-deep-research` as far as it can be one; seed 002 lays down the rare-earth supply chain as stages, elements, materials and typed relationships; `services/analytics/src/mineral_analytics/concentration.py` computes the bottleneck indices; and `packages/db/src/ontology-repository.ts` reads the chain, records a measured quantity and stores an index, rendered at `/supply-chain/[code]`. Six decisions are worth recording. `CORE_RECIPE` is left as it was and the new modules ride in `DEEP_RECIPE` instead, because core is the cheap path the web trigger takes and fifteen model calls is not cheap; a recipe is an ordered set of modules and having two of them is what that design is for. `bear_case` had to move, which is a rule change under brief section 24: the registry made it depend on `valuation_calc`, which is not a DAG module here but a step in the decision layer, so no recipe could ever satisfy it; it now depends on `valuation_assumptions`, which is both reachable and better, since a bear case that attacks the discount rate is attacking what the valuation is made of rather than disputing its total. The chain is seeded rather than extracted, and the seed contains no quantity at all: stages and flows are definitions, while who produces how much is measurement that has to arrive with a source under it, so seed 002 says what a separation plant is and says nothing about anyone's output. Concentration indices are stored as `valuation.calculation_runs` rows with a `calculation_run_inputs` row per producer revision, and not promoted as `CALCULATED` facts: the run already carries the provenance an index needs, and a fact on the material would claim the index was a property of the material rather than of a market. That required widening the method check constraint, migration `20260918000000_concentration_method.sql`, which is the second rule change here: computing the index while rendering instead would have broken invariant C.9. Production quantities enter through `recordProduction`, which writes `extraction_method = 'manual'` and demands a source document version rather than accepting a null one, because XBRL carries financial statements and says nothing about tonnes, and the model extraction that would read a production table out of prose is report D.2b and is not built; until it is, a tonnage enters the way a person would enter one, with its filing attached. And a stage nobody has measured renders as a gap rather than a small number, which is the whole posture of the page: a bottleneck is the easiest claim in this domain to assert and the hardest to support, so the page shows nothing until something has been measured. Phase J.12 (evals) is implemented: `evals/` holds two labelled datasets and the runner behind `pnpm eval`, `evals/src/score.ts` turns expected and actual verdicts into accuracy, precision and recall, and a new CI job runs the suite on every pull request. Five decisions are worth recording. The suites are hermetic, with no database and no model key, because CI has neither and an eval that only runs where they exist is not an eval that runs on a pull request; what they measure is therefore the deterministic layer, the verification rules and the XBRL concept map, and scoring a model's judgement waits for the phase that can record cached model runs to score against. Roughly half of every rule's cases are clean claims, because a rule measured only on cases it should fail proves nothing about false positives: precision and recall are both reported, and a shipped test asserts that no rule is measured on a single case. The threshold lives in the dataset rather than in a separate baselines file, since two places to record the same number is one place to forget it, and a run below the threshold exits non-zero, which is what gates a module version bump. Temporal correctness arrived as a real rule rather than as a dataset: `date_consistency` was a declared check type that nothing emitted, and it now checks the years a statement names against the years its evidence carries, including the period of a cited fact and the publication date of a cited filing. It warns rather than errors, which is a judgement and not a hedge: a filing saying the year ended December 31 2025 against a claim saying fiscal 2025 is a real false positive the rule cannot distinguish, so it reports without demoting. And the eval earned itself immediately, which is the argument for having written it: the contradiction rule emitted a check only when it fired, so two cases scored `absent` rather than `passed`, and a run where nothing disagreed was indistinguishable in `research.verification_checks` from a run where that rule never executed. It now writes a verdict for every claim, which is what makes a stored check evidence that the check happened.
